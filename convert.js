...
var add_inputs, available_chars, blank, cancel, control, curse, down, draw, enter, esc, input, keymap, left, ll, render_curse, right, space, store, tab, tag, up,
  __slice = [].slice,
...
space = function() {
  var recurse;
  recurse = function(arr) {
    var copy, curse_index, item, _i, _len;
    copy = [];
    for (_i = 0, _len = arr.length; _i < _len; _i++) {
      item = arr[_i];
      if (Array.isArray(item)) {
        ...
        if (curse_index === -1) {
          ...
          copy.push(item.slice(0, curse_index).concat(item.slice(curse_index + 1)));
          copy.push(curse);
        }
      }
    }
